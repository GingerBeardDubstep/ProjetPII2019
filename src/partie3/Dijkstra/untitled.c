...
int main(int argc, char const *argv[])
{
    ...
    if (dep == -1 || dest == -1){
        ...
        chargeTableAeroport("airports.csv", table, 2822);
        chargeTableNode("routes.csv", table, donnees, 7526);
        dep = airport_getNum(table, dep, 7526);
        dest = airport_getNum(table, dest, 7526);
        printf("%d %d\n",dep, dest );
        node * marchestp = astar(donnees, dep, dest, table, 7526);
        if (marchestp == NULL){
